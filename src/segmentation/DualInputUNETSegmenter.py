"""
this is the implemnetatin of Unet segmentation
code is baed on :
https://github.com/dsp-uga/team-ball/blob/master/src/Classifiers/UNet_Classifier.py
...
class Dual_Input_UNET_Segmenter(Segmenter):
    """
    ...
    def __init__(self, loss_function="dice_coef"):
        ...
        self.classifier_name = "DUALINPUTUNET"
        self.data = None

        if (loss_function == "dice_coef"):
            self.metrics_function = Dual_Input_UNET_Segmenter.dice_coef
            self.loss_function = Dual_Input_UNET_Segmenter.dice_coef_loss
        elif loss_function == "dice_coef2":
            self.metrics_function = Dual_Input_UNET_Segmenter.dice_coef2
            self.loss_function = Dual_Input_UNET_Segmenter.dice_coef_loss2
    ...
    def dice_coef_loss(y_true, y_pred):
        return - Dual_Input_UNET_Segmenter.dice_coef(y_true, y_pred)
    ...
    def load_model(self, import_path):
        ...
        self.trained_model = load_model(os.path.join(import_path, self.classifier_name + ".h5"),
                                        custom_objects={
                                                        'dice_coef_loss': Dual_Input_UNET_Segmenter.dice_coef_loss,
                                                        'dice_coef': Dual_Input_UNET_Segmenter.dice_coef})
        logging.info("Loaded Model at : " + os.path.join(import_path, self.classifier_name + ".h5"))

    def train(self, x_train, y_train, v_train, epochs=1200, batch_size=4):
        """
        ...
        model = Model(inputs=[main_input , var_input], outputs=[conv10])
        model.compile(optimizer=Adam(lr=1e-5), loss=Dual_Input_UNET_Segmenter.dice_coef_loss, metrics=[Dual_Input_UNET_Segmenter.dice_coef])
        print(model.summary())

        # training network
        model.fit([x_train, v_train], [y_train], batch_size=batch_size, epochs=epochs, shuffle=True)
