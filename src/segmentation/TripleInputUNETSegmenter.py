...
class Triple_Input_UNET_Segmenter(Segmenter):
    """
    ...
    def __init__(self, loss_function="dice_coef"):
        ...
        self.classifier_name = "TRIPPLEINPUTUNET"
        self.data = None

        if (loss_function == "dice_coef"):
            self.metrics_function = Triple_Input_UNET_Segmenter.dice_coef
            self.loss_function = Triple_Input_UNET_Segmenter.dice_coef_loss
        elif loss_function == "dice_coef2":
            self.metrics_function = Triple_Input_UNET_Segmenter.dice_coef2
            self.loss_function = Triple_Input_UNET_Segmenter.dice_coef_loss2
    ...
    def dice_coef_loss(y_true, y_pred):
        return - Triple_Input_UNET_Segmenter.dice_coef(y_true, y_pred)
    ...
    def load_model(self, import_path):
        ...
        self.trained_model = load_model(os.path.join(import_path, self.classifier_name + ".h5"),
                                        custom_objects={
                                                        'dice_coef_loss': Triple_Input_UNET_Segmenter.dice_coef_loss,
                                                        'dice_coef': Triple_Input_UNET_Segmenter.dice_coef})
        logging.info("Loaded Model at : " + os.path.join(import_path, self.classifier_name + ".h5"))

    def train(self, x_train, y_train, v_train, of_train, epochs=1200, batch_size=4):
        """
        ...
        if self.trained_model is None:
            merger = concatenate([main_input , var_input, of_input])
